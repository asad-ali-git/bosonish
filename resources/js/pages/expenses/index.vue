  ...
  <div class="container px-6 mx-auto py-8 text-gray-600">
    <div class="flex flex-wrap">
      ...
      <div
        ...
        "
      >
        <div class="flex items-center" v-if="selectedRows.length > 0">
        ...
        <t-button
          ...
          @click="showExpenseSidePanel = true"
        >
          <IconBase
            class="w-4 h-4"
            viewBox="0 0 16 16"
          ...
          />
          <span class="mr-2">Add Expenses</span>
        </t-button>
      </div>
    </div>
    <DataTable
    ...
    <SidePanel v-model="showExpenseSidePanel" :record="expense" @submitted="getData"/>
  </div>
</template>
...
export default {
  ...
  methods: {
    async getData() {
      ...
      editRecord(row){
      this.showExpenseSidePanel = true;
      this.expense=row
    },
    deleteRecord (row) {
      this.$dialog.confirm({
